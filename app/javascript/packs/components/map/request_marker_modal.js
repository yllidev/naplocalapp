...
class RequestMarkerModal extends React.Component {
    ...
    checkIfUserResponded = (data) => {
        for(let i=0; i<data.length; i++){
            // we don't have volunteer_id in convrsation so we will use user_id
            if (data[i].user_id === Cookies.getJSON('currentUser').user_id){
                this.setState({
    ...
    submitMessage = (conversationId) => {
        ...
        fetch(url, {
            ...
            if(response.status === 201) {
                ...
                this.fetchRepliesNumber(this.props.request.id)
                this.props.setMessageIndicator(true)
                this.setState({
                    open: true
                })
            }
